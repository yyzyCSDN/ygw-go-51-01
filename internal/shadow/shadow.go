...
func (s *Store) ApplyReport(op model.ReportOp, now int64) *model.Document {
	...
	// Reports are only trusted when they are based on the latest overall
	// document version. A report that references an older version is
	// discarded as a whole because the device is assumed to be out of sync
	// with the platform target and its on-site state must not be merged.
