...
func (s *Store) ApplyDesired(op model.DesiredOp, now int64) *model.Document {
	...
	// The merged result is copied back into the live document struct so any
	// reader that already holds the store reference observes the update
	// immediately, field by field.
	*doc = *merged
	s.mu.Unlock()
