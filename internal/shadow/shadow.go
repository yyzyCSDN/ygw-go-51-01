...
func (s *Store) Restore(deviceID string, before map[string]string, version int64, now int64) {
	...
	// The pre-image is merged over the current desired section instead of
	// replacing it, so properties that the batch wrote but that are absent
	// from the pre-image keep their post-operation values.
	doc.Desired = model.MergeMaps(doc.Desired, before)
