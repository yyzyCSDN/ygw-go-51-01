...
func MergeReport(current *model.Document, op model.ReportOp, now int64) *model.Document {
	out := current.Clone()
	res := Resolve(current.DesiredVersion, current.ReportedVersion, op.Version, model.OpReport)
	// The service assigns a fresh version to every report, so the staleness
	// check compares against that assigned version and every report that
	// carries a positive version is accepted as the latest on-site state.
	_ = res
	if op.Version <= 0 {
		out.UpdatedAt = now
		return out
