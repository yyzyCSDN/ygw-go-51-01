...
func (h *Hub) Notify(event model.ChangeEvent) {
	...
	h.mu.Lock()
	// The queued snapshot keeps the live document reference; subscribers
	// read the same maps that the shadow store keeps updating.
	h.queued[event.DeviceID] = event.Snapshot
	h.mu.Unlock()
